...
- Bounded record-storage protocol 1.1 with revisioned reads, opaque cursor pages, and atomic idempotent multi-record transactions.
- Feature-gated immutable event publication and bounded reads in D1 with hash-only idempotency, exact type filtering, opaque cursors, cancellation-aware long polling, finite quotas, retention, and rate controls.
...
- `BOUNDED_RECORD_RECEIPT_GLOBAL_MAX_*`, `BOUNDED_RECORD_RECEIPT_USER_MAX_*`, and `BOUNDED_RECORD_RECEIPT_NAMESPACE_MAX_*`: independent finite ordinary transaction-receipt count and result-byte ceilings. Delete-only transactions may use a separate server-derived reserve tied to storage item ceilings so authorized cleanup remains possible without permitting unbounded create/delete receipt growth.
...
- `GET /storage/record-protocol`
- `GET /storage/record-protocol/records`
- `GET /storage/record-protocol/records/{collection}/{id}`
- `POST /storage/record-protocol/transactions`
- `GET /storage/files`
- `POST /storage/files`
...
The `/storage/records`, `/storage/record-protocol`, and `/storage/files` browser views default to the current signed-in session when one exists. They create only a minimal short-lived internal access token, pass it directly to the canonical storage validator, and never expose or retain it. When OAuth Apps are enabled, `/userinfo` offers the equivalent current-session adapter and explicit AittaDB bearer-token mode; when disabled, UserInfo disappears from the session representations and rejects before token or identity lookup. Inactive token and OAuth grant fields are hidden and disabled by progressive enhancement; without JavaScript, every field remains visible and server validation is authoritative. Storage collection pages render a useful list or empty state. The bounded protocol page also renders real read/list and CSRF-protected transaction forms. The file collection provides an accessible native upload control with drag-and-drop enhancement. After a successful signed-in upload, the browser follows a validated `303` to the canonical item page, so refresh cannot resubmit the upload. Item pages render readable details and only actions valid for that exact URL instead of dumping JSON into the HTML shell.
...
Storage is isolated by the immutable AittaDB user UUID and OAuth client ID. Legacy JSON records are stored in D1 at `/storage/records/{key}`. The discovery resource at `/storage/record-protocol` additionally exposes revisioned bounded records and atomic compare-and-set transactions; clients should follow its advertised targets. File metadata is stored in D1 and file bytes are stored in R2. `POST /storage/files` creates a file with a server-generated logical key. Bearer JSON/hypermedia and explicit-token HTML responses return `201 Created` with the item URI in `Location`; signed-in HTML accepts that URI only from the request or configured issuer origin and redirects with `303 See Other` on the request origin. `PUT /storage/files/{key}` creates or replaces the file at a caller-selected logical key. Caller-provided keys are logical metadata only; AittaDB always generates physical R2 object keys. See the [bounded record protocol](docs/bounded-record-storage-protocol.md) and [request examples](docs/examples.md#atomic-bounded-records).
