...
Request bodies have finite route-specific limits. AittaDB bounds accepted URL-encoded forms and JSON records from the actual stream before parsing or database access; `Content-Length` can reject a declared overflow early but is never trusted to make the streamed body smaller. Oversized input returns the documented `413 invalid_request`, and a malformed request stream returns `400 invalid_request` without durable changes.
