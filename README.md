...
- `FEATURE_RECORDS_ENABLED`, `FEATURE_FILES_ENABLED`, `FEATURE_STATISTICS_ENABLED`, and `FEATURE_OAUTH_APPS_ENABLED`: strict per-feature booleans. Records, Files, and Statistics default on; downstream OAuth Apps default off. The effective state is visible in the service HTML and versioned hypermedia metadata, without exposing raw environment configuration. Setting `FEATURE_RECORDS_ENABLED=false` removes JSON Records controls from service, session, and file-storage discovery and makes every `/storage/records` operation return content-negotiated `503 feature_unavailable` before authentication, rate limiting, body parsing, or repository access. When Statistics is off, the root omits its link and action, and `GET /statistics` returns the same negotiated error before querying D1. Other enabled features remain available.
