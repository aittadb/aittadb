...
- `FEATURE_RECORDS_ENABLED`, `FEATURE_FILES_ENABLED`, `FEATURE_STATISTICS_ENABLED`, and `FEATURE_OAUTH_APPS_ENABLED`: strict per-feature booleans. Records, Files, and Statistics default on; downstream OAuth Apps default off. The effective state is visible in the service HTML and versioned hypermedia metadata, without exposing raw environment configuration. When Statistics is off, the root omits its link and action, and `GET /statistics` returns a content-negotiated `503 feature_unavailable` response before querying D1.
