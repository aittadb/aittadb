...
test("D1 and memory resolve an existing user by exact email without mutation", async (t) => {
  for (const adapter of adapters()) {
    await t.test(adapter.name, async () => {
      const fixture = await adapter.create();
      try {
        const identity = {
          email: `${adapter.name}-lookup@example.test`,
          fullName: "Original Name",
          displayName: "Original Name",
        };
        const created = await fixture.store.findOrCreateUser(identity, 20);

        assert.deepEqual(
          await fixture.store.getUserByEmail(identity.email),
          created,
        );
        assert.equal(
          await fixture.store.getUserByEmail(identity.email.toUpperCase()),
          null,
        );
        assert.equal(
          await fixture.store.getUserByEmail("missing@example.test"),
          null,
        );
        assert.deepEqual(await fixture.store.getUser(created.id), created);
      } finally {
        fixture.close();
      }
    });
