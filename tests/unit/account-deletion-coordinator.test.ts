...
import {
  ACCOUNT_DELETION_COORDINATOR_DEFERRED_EVENT,
  ACCOUNT_DELETION_COORDINATOR_FAILURE_EVENT,
...
class InterruptingMemoryStore extends MemoryAuthStore {
  ...
  constructor(private readonly phase: AccountDeletionCoordinatorFailurePhase) {
    super();
  }
  ...
  private interrupt(phase: AccountDeletionCoordinatorFailurePhase): void {
...
class DeferringMemoryStore extends MemoryAuthStore {
  deferred = false;

  constructor(private readonly phase: AccountDeletionCoordinatorDeferredPhase) {
    super();
  }
  ...
  override async purgeAccountCredentialsAndGrants(
    ...
    const result = await super.purgeAccountCredentialsAndGrants(subject, limit);
    if (!this.deferred && this.phase === "credentials") {
      this.deferred = true;
  ...
  override async hasStorageFilesForSubject(subject: string): Promise<boolean> {
    const result = await super.hasStorageFilesForSubject(subject);
    if (!this.deferred && this.phase === "files") {
      this.deferred = true;
      return true;
    }
    return result;
  ...
  override async finalizeAccountDeletion(
    ...
    if (!this.deferred && this.phase === "finalization") {
      this.deferred = true;
      return false;
    }
    return super.finalizeAccountDeletion(subject, attempt, now);
  }
}

class CredentialFailingMemoryStore extends MemoryAuthStore {
  override async purgeAccountCredentialsAndGrants(): Promise<AccountCredentialPurgeBatchResult> {
    throw new AccountCredentialPurgeFailure("device-grants");
  }
}
  ...
  for (const name of ["memory", "d1"] as const) {
    await t.test(name, async () => {
      ...
      try {
        ...
        await assertSecurityRetention(fixture, clientHash, rateKey, target);
...
test("coordinator resumes after interruption following every phase", async (t) => {
  for (const phase of [
    ...
    await t.test(phase, async () => {
      ...
      await store.audit("interruption-attribution", { phase }, 3, {
        actorSubjectHash: await sha256(target.id),
      });
      await store.startAccountDeletionJob(target.id, 400);

      let now = 400;
      const observed: AccountDeletionCoordinatorFailurePhase[] = [];
      const observeFailure = (
        observedPhase: AccountDeletionCoordinatorFailurePhase,
      ) => {
        observed.push(observedPhase);
      ...
      while (result.completed === 0) {
        now += ACCOUNT_DELETION_COORDINATOR_RETRY_SECONDS;
        result = await coordinateAccountDeletionBatch(
          store,
          bucket,
          () => now,
          observeFailure,
      ...
      assert.deepEqual(observed, [phase]);
      ...
      assert.equal(store.audits[0]!.actorSubjectHash, null);
    });
  }
});

test("coordinator failure telemetry has one fixed redacted shape", () => {
  for (const phase of [
    "fences",
    "credentials",
    "records",
    ...
    const telemetry = accountDeletionCoordinatorFailureTelemetry(phase);
    assert.deepEqual(JSON.parse(telemetry), {
      event: ACCOUNT_DELETION_COORDINATOR_FAILURE_EVENT,
      phase,
    });
    assert.deepEqual(Object.keys(JSON.parse(telemetry)).sort(), [
      "event",
      "phase",
    ]);
  }
...
test("coordinator deferred telemetry has one fixed redacted shape", () => {
  for (const phase of [
    "credentials",
    "records",
    "files",
    ...
    const telemetry = accountDeletionCoordinatorDeferredTelemetry(phase);
    assert.deepEqual(JSON.parse(telemetry), {
...
test("credential subphase observer failure cannot change durable retry", async () => {
  const store = new CredentialFailingMemoryStore();
  const bucket = new RetryingBucket();
  const target = await createUser(store, "credential-subphase", 1);
  await store.startAccountDeletionJob(target.id, 100);
  const broad: AccountDeletionCoordinatorFailurePhase[] = [];
  const subphases: AccountCredentialPurgeFailurePhase[] = [];

  const result = await coordinateAccountDeletionBatch(
    store,
    bucket,
    () => 100,
    (phase) => broad.push(phase),
    undefined,
    (phase) => {
      subphases.push(phase);
      throw new Error("synthetic_subphase_observer_failure");
    },
  );

  assert.equal(result.deferred, 1);
  assert.deepEqual(broad, ["credentials"]);
  assert.deepEqual(subphases, ["device-grants"]);
  assert.equal(
    (await store.getAccountDeletionJob(target.id))?.state,
    "retryable",
  );
  assert.ok(await store.getUser(target.id));
});

test("coordinator reports the first incomplete phase without changing progress", async (t) => {
  for (const phase of [
    "credentials",
    "records",
    "files",
    ...
    await t.test(phase, async () => {
      const store = new DeferringMemoryStore(phase);
      const bucket = new RetryingBucket();
      const target = await createUser(store, `deferred-${phase}`, 1);
      await store.startAccountDeletionJob(target.id, 100);
      const observed: AccountDeletionCoordinatorDeferredPhase[] = [];
      const deferred = await coordinateAccountDeletionBatch(
        ...
        (observedPhase) => {
          observed.push(observedPhase);
          throw new Error("synthetic_deferred_observer_failure");
        },
      );
      ...
      const completed = await coordinateAccountDeletionBatch(
        store,
        bucket,
        () => 100 + ACCOUNT_DELETION_COORDINATOR_RETRY_SECONDS,
      );
...
test("coordinator reports no failure for a successful pass", async () => {
  const store = new MemoryAuthStore();
  const bucket = new RetryingBucket();
  const target = await createUser(store, "telemetry-success", 1);
  await store.startAccountDeletionJob(target.id, 100);
  const observed: AccountDeletionCoordinatorFailurePhase[] = [];
...
test("audit repositories reject noncanonical actor attribution before mutation", async (t) => {
  for (const name of ["memory", "d1"] as const) {
    await t.test(name, async () => {
      const fixture = await createFixture(name, true);
      try {
        for (const actorSubjectHash of [
          "",
          "a".repeat(42),
          "a".repeat(44),
          `${"a".repeat(42)}=`,
          ...
          await assert.rejects(
            fixture.store.audit("invalid-attribution", { keep: true }, 1, {
              actorSubjectHash,
            }),
            /audit_actor_subject_hash_invalid/,
          );
        }
        assert.equal(await auditEventCount(fixture), 0);

        const validActor = `${"A-_".repeat(14)}A`;
        await fixture.store.audit("valid-attribution", { keep: true }, 1, {
          actorSubjectHash: validActor,
        });
        assert.equal(await auditEventCount(fixture), 1);
        assert.deepEqual(await attributedAuditTypes(fixture, validActor), [
          "valid-attribution",
        ]);
      } finally {
        fixture.close();
...
test("finalization unlinks at most 100 deterministic audit attributions", async (t) => {
  for (const name of ["memory", "d1"] as const) {
    for (const attributedCount of [100, 101] as const) {
      await t.test(`${name}-${attributedCount}`, async () => {
        const fixture = await createFixture(name, true);
        try {
          const target = await createUser(
            fixture.store,
            `${name}-audit-${attributedCount}`,
            1,
          );
          const control = await createUser(
            fixture.store,
            `${name}-audit-control-${attributedCount}`,
            2,
          );
          const targetHash = await sha256(target.id);
          const controlHash = await sha256(control.id);
          for (let index = 0; index < attributedCount; index += 1) {
            await fixture.store.audit(
              `target-${index}`,
              { sequence: index },
              10,
              { actorSubjectHash: targetHash },
            );
          }
          await fixture.store.audit("control", { keep: true }, 5, {
            actorSubjectHash: controlHash,
          });
          const rateKey = `audit-unlink:${"r".repeat(43)}`;
          assert.equal(
            await fixture.store.rateLimit(rateKey, 5, 60, 100),
            true,
          );
          ...
          assert.ok(claim);
          const first = await fixture.store.finalizeAccountDeletion(
            target.id,
            claim.attempt,
            100,
          );

          assert.equal(first, attributedCount === 100);
          assert.equal(await auditEventCount(fixture), attributedCount + 1);
          assert.deepEqual(await attributedAuditTypes(fixture, controlHash), [
            "control",
          ]);
          assert.equal(await rateCounterExists(fixture, rateKey), true);

          if (attributedCount === 101) {
            assert.ok(await fixture.store.getUser(target.id));
            assert.deepEqual(await attributedAuditTypes(fixture, targetHash), [
              "target-100",
            ]);
            assert.equal(
              await fixture.store.finalizeAccountDeletion(
  ...
  for (const name of ["memory", "d1"] as const) {
    await t.test(name, async () => {
      ...
      try {
        ...
        const targetHash = await sha256(target.id);
        await fixture.store.audit("stale-attribution", { retained: true }, 2, {
          actorSubjectHash: targetHash,
        });
        await fixture.store.startAccountDeletionJob(target.id, 100);
...
test("D1 uncertain finalization commits are observed as completed", async () => {
  ...
  try {
    ...
    const targetHash = await sha256(target.id);
    await fixture.store.audit("uncertain-attribution", { retained: true }, 2, {
      actorSubjectHash: targetHash,
    });
    await fixture.store.startAccountDeletionJob(target.id, 100);
    const result = await coordinateAccountDeletionBatch(
...
async function assertSecurityRetention(
  ...
  let actorSubjectHash: string | null;
  if (fixture.store instanceof MemoryAuthStore) {
    assert.equal(fixture.store.audits.length, 1);
    auditData = fixture.store.audits[0]!.data;
    actorSubjectHash = fixture.store.audits[0]!.actorSubjectHash;
  } else {
    const row = fixture
      .sqlite!.prepare(
        "SELECT data_json, actor_subject_hash FROM audit_events ORDER BY id ASC LIMIT 1",
      )
      .get();
    assert.ok(row);
    auditData = JSON.parse(String(row.data_json)) as Record<string, unknown>;
    actorSubjectHash =
      typeof row.actor_subject_hash === "string"
        ? row.actor_subject_hash
        : null;
  }
  assert.deepEqual(auditData, { clientHash });
  assert.equal(actorSubjectHash, null);
...
async function attributedAuditTypes(
  fixture: Fixture,
  actorSubjectHash: string,
): Promise<string[]> {
  if (fixture.store instanceof MemoryAuthStore) {
    return fixture.store.audits
      .filter((event) => event.actorSubjectHash === actorSubjectHash)
      .map((event) => event.type);
  }
  return fixture
    .sqlite!.prepare(
      "SELECT type FROM audit_events WHERE actor_subject_hash = ? ORDER BY created_at ASC, id ASC",
    )
    .all(actorSubjectHash)
