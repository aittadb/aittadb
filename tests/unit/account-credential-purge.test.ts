...
import {
  ACCOUNT_CREDENTIAL_PURGE_FAILURE_EVENT,
...
test("credential purge failures expose only a fixed subphase", async (t) => {
  const cases: Array<{
  ...
  }> = [
    {
      phase: "job",
      method: "first",
      query: "FROM account_deletion_jobs WHERE subject = ?",
    },
    {
      phase: "authorization-codes",
      method: "run",
      query: "DELETE FROM authorization_codes",
    },
    {
      phase: "authorization-requests",
      method: "run",
      query: "DELETE FROM authorization_requests",
    },
    {
      phase: "device-grants",
      method: "run",
    ...
    },
    {
      ...
      method: "run",
      query: "DELETE FROM refresh_token_families",
    },
    {
      phase: "access-revocations",
      method: "run",
      query: "DELETE FROM revoked_access_tokens",
    },
    {
      phase: "admin-submissions",
      method: "run",
      query: "DELETE FROM admin_operation_submissions",
    },
    {
      phase: "remaining-check",
      method: "first",
      query: "SELECT 1 AS remaining",
    },
  ];

  for (const fixture of cases) {
    await t.test(fixture.phase, async () => {
      const sqlite = await migratedDatabase();
      try {
        const base = sqliteD1(sqlite);
        const setup = new D1AuthStore(base);
        const subject = await createSubject(
          setup,
          `subphase-${fixture.phase}`,
          1,
        );
        await setup.startAccountDeletionJob(subject, 2);
        const store = new D1AuthStore(
          failOperationOnce(
            base,
            fixture.method,
            fixture.query,
            "synthetic_private_adapter_failure",
          ),
        );
        ...
        await assert.rejects(
          ...
          (error: unknown) =>
            error instanceof AccountCredentialPurgeFailure &&
            error.phase === fixture.phase &&
            error.message === "account_credential_purge_failed" &&
            !error.message.includes("synthetic_private_adapter_failure"),
        );
      } finally {
        sqlite.close();
      }
...
test("credential purge telemetry has one fixed redacted shape", () => {
  for (const phase of [
    "job",
    "authorization-codes",
    ...
    const telemetry = accountCredentialPurgeFailureTelemetry(phase);
...
test("D1 credential remaining check avoids a compound SELECT", async () => {
  const sqlite = await migratedDatabase();
  try {
    const base = sqliteD1(sqlite);
    const setup = new D1AuthStore(base);
    const subject = await createSubject(setup, "single-select-remaining", 1);
    await setup.startAccountDeletionJob(subject, 2);
    const store = new D1AuthStore(rejectQueryFragment(base, "UNION ALL"));

    assert.deepEqual(
      await store.purgeAccountCredentialsAndGrants(subject, 100),
      { deletedCount: 0, done: true },
    );
  } finally {
    sqlite.close();
  ...
  return {
    prepare(query: string): D1PreparedStatement {
      ...
      const statement: D1PreparedStatement = {
        ...
        async first<T>(): Promise<T | null> {
          if (!failed && method === "first" && query.includes(queryFragment)) {
            failed = true;
            throw new Error(errorMessage);
          }
        ...
        all: <T>() => bound.all<T>(),
        async run<T>(): Promise<D1Result<T>> {
          if (!failed && method === "run" && query.includes(queryFragment)) {
            failed = true;
            throw new Error(errorMessage);
          }
          return bound.run<T>();
  ...
  return {
    prepare(query: string): D1PreparedStatement {
      if (query.includes(queryFragment)) {
        throw new Error("synthetic_sites_compound_query_rejection");
      }
