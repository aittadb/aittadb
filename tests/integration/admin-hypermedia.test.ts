...
test("admin HTML and hypermedia expose the same state- and type-aware controls", async () => {
  ...
  const activeService = await createClientRegistration(
    {
      type: "service",
      name: "Active service",
      redirectUris: [],
  ...
  assert.match(html, /<option value="service">service<\/option>/);
  ...
  for (const [clientId, expected] of [
    ...
    [activeService.client.id, ["disable", "rotate_secret", "revoke_grants"]],
  ] as const) {
...
test("secret-bearing client creation and rotation reveal a client-bound secret only once", async () => {
  const fixture = await adminFixture();
  ...
  const jsonService = await adminPost(
    fixture,
    jsonCsrf,
    documentSubmissionToken(jsonConfidentialDocument),
    {
      name: "JSON service",
      type: "service",
      redirect_uris: "",
  ...
  assert.equal(jsonService.status, 200);
  const jsonServiceDocument = (await jsonService.json()) as AdminDocument;
  const serviceSecret = jsonServiceDocument.data.new_client_secret;
  const serviceClientId = jsonServiceDocument.data.new_client_secret_client_id;
  assert.match(serviceSecret ?? "", /^[A-Za-z0-9_-]{43}$/);
  assert.equal(
    await fixture.store.getClientSecretHash(serviceClientId!),
    await sha256(serviceSecret!),
  );
  assert.equal(await fixture.store.hasServicePrincipal(serviceClientId!), true);
  assert.equal(
    jsonServiceDocument.data.clients.find(
      (client) => client.id === serviceClientId,
    )?.type,
    "service",
  );
  const jsonLater = await adminGet(fixture);
  const jsonLaterText = await jsonLater.text();
  assert.doesNotMatch(jsonLaterText, new RegExp(jsonSecret!));
  assert.doesNotMatch(jsonLaterText, new RegExp(serviceSecret!));
});
