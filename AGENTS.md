...
The server boundary includes identity/authentication; user/client/application/namespace isolation; records, objects, events, and delivery; conditional writes, versions, cursors, idempotency, bounded atomic operations, quotas, limits, expiry, retention, cleanup, hypermedia, OpenAPI, and protocol/operations docs. Client SDKs, libraries, application integrations, and provider adapters belong in separate repositories; this repository documents protocols but does not own clients.

Application workflows such as billing, memberships, provisioning, messaging, or games belong outside AittaDB and compose its primitives. Examples may motivate/evaluate a primitive but cannot determine provider-specific routes, schemas, configuration, scopes, or business rules here.
...
- Account deletion: any job blocks access; admins cannot start. One claimed job advances bounded fence (25), credential (100), record (100), and file (25) work. R2 puts hold durable fences; expiry becomes repair. Only its exact live attempt may atomically remove a clean user and complete the minimal UUID tombstone. Foreign references block. Progress is aggregate/internal; no route yet.
...
The public root returns concise hypermedia JSON or a polished operation map, never marketing or a fake demo. Add no hero, pricing, testimonials, blog, dashboard, general account/profile pages, or nonessential navigation. `/session` is a focused protected identity/operations view.
