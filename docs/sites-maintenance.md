...
Receipt counts are the bounded preflight/postflight semantic count, not a raw D1 mutation-metadata total: deleting one receipt may cascade its private receipt-collection index row. This keeps the count-only result portable without treating an engine-specific cascade total as a second user-visible receipt.
