...
- `src/storage.ts`: broker-local storage endpoints and scope enforcement.
...
The service stores local structured state in D1. JSON storage records are D1 rows keyed by local user UUID, OAuth client ID, and logical app key. File metadata is stored in D1 while file bytes are stored in R2 through binding `BUCKET`; physical R2 keys are generated by the broker and are not caller-controlled.
