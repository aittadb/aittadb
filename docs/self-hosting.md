...
Self-hosted operators must configure their own finite storage ceilings and write kill switch; AittaDB's defaults are not claims about the replacement provider's capacity. They must also provide HTTPS/HSTS at the effective public edge, streaming body limits, exact CORS behavior, an independent administrator access-key hash, and immutable local UUID administrator subjects. Email allowlisting is only a temporary bootstrap path, but UUID allowlisting still inherits reassignment risk until the upstream mapping has a stable subject. The independent key remains the separate administrator factor.
