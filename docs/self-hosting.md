...
AittaDB is a third-party project, not an OpenAI product. Its current implementation depends on OpenAI-hosted ChatGPT Sites unless an operator replaces the platform-specific runtime, identity, storage, configuration, and secret adapters. Current releases are source-available under FSL-1.1-MIT and each released version converts to the MIT License two years after publication, regardless of hosting environment.
