...
Users can delete individual records and files when their current session or approved client has the required permission. The MVP still has no self-service account-deletion operation; requests concerning the local identity or remaining data must be sent to the published privacy contact. A durable internal account-deletion job plus bounded credential/grant and JSON-record purges exist for the future workflow, but none has an HTTP operation or coordinator. Every job state blocks new AittaDB sessions and tokens, refresh successors, existing access-token use, and storage access for the local subject. Internal batches can remove only that subject's attributed OAuth transaction, consent, refresh, revocation, and D1 record state across client namespaces. They do not remove files, identity, or the terminal tombstone, and responses expose neither job nor purge progress.
...
A bounded internal file purge exists for future whole-account deletion coordination. It accepts only an exact actively claimed deletion job, removes metadata across that subject's client namespaces in finite batches, and routes object deletion through private persistent repair state. It publishes no file, key, subject, or progress resource and is not reachable through HTTP. A completed job permits only a clean idempotent no-op.
