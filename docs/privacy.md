...
Users can delete individual records and files when their current session or approved client has the required permission. The MVP still has no self-service whole-account deletion operation; requests concerning the local identity or remaining data must be sent to the published privacy contact. Internal primitives can durably start and coordinate bounded deletion of one local subject's attributed OAuth state, administrator submissions, JSON records, file metadata, R2 objects, and local user row. Every job state blocks new AittaDB sessions and credentials, refresh successors, existing access-token use, and storage access for that subject. The coordinator has no HTTP/status representation, and its progress exposes no personal or storage identifiers.

Records and files remain until an authorized user deletes them, an internally initiated deletion job completes, or the operator ends or removes the deployment, subject to provider lifecycle, backups, legal duties, and technical recovery periods. OAuth access-token, authorization-code, device-code, and refresh-token lifetimes are configurable; expired protocol state becomes eligible for bounded cleanup. One-time administrator submission hashes become cleanup-eligible after 15 minutes, audit events after 90 days, and one-minute rate counters after five minutes. During account deletion, owned administrator submissions are removed explicitly. Existing audit rows retain only their ordinary bounded, redacted security data; storage rate keys are SHA-256 owner hashes and remain only until the five-minute cleanup threshold. Neither is enriched with deletion details.

The completed deletion-job row is a permanent pseudonymous security/idempotency exception. It retains the former local UUID, coarse state, attempt, and timestamps so old credentials stay invalid and repeat deletion remains terminal. It has no email, display name, content, client identifier, credential, logical or physical storage key, subject-derived content hash, or repair detail, and it has no public representation. This UUID remains linkable and is not anonymous. Revocation rows created before subject attribution cannot be safely assigned to an account, so deletion retains them until ordinary expiry cleanup. Cleanup is bounded, scheduled outside responses, and depends on traffic and platform execution, so eligibility is not a guarantee of immediate physical deletion.

File deletion accepts only an exact actively claimed job, removes metadata across that subject's client namespaces in finite batches, and routes object deletion through private persistent repair state. In-flight uploads reserve durable fences before R2 writes; deletion waits until each fence clears or atomically becomes repair work. R2 physical keys and custom metadata contain no subject, client, or logical key. Foreign-owner references and unresolved retirement block final user removal. Finalization removes personal fields and the local user only after every attributed row, fence, and repair is gone.
