...
| Event cursors                                 | Existing 15-minute Events checkpoints become invalid when either the private key or key ID changes. Restart from the Events collection entry point. Rolling back to the exact old key and ID may make an otherwise-unexpired cursor readable again, but callers must treat every pre-cutover cursor as retired.                             |
