...
Leave `FEATURE_OAUTH_APPS_ENABLED=false` until downstream third-party OAuth applications are intentionally enabled. Client administration, `/authorize`, `/oauth/device_authorization`, their `/device`, `/device/decision`, and `/consent` continuations, and `/oauth/token` then reject before body reading, identity, CORS/client/credential lookup, rate limiting, cleanup, or mutation. Application resources return content-negotiated `503 feature_unavailable`; token POST returns no-store OAuth `temporarily_unavailable`. Their service controls disappear. This does not disable the private reserved client used for AittaDB's own signed-in session and storage operations; remaining token-lifecycle and discovery gates are documented with the tasks that own them.
