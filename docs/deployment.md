...
Leave `FEATURE_OAUTH_APPS_ENABLED=false` until downstream third-party OAuth applications are intentionally enabled. Client-administration GET and POST then return content-negotiated `503 feature_unavailable` before identity lookup, repository access, rate limiting, body parsing, cleanup, or mutation, and administrator controls disappear from service and session representations. This does not disable the private reserved client used for AittaDB's own signed-in session and storage operations.
