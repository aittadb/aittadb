...
Leave `FEATURE_OAUTH_APPS_ENABLED=false` until downstream third-party OAuth applications are intentionally enabled. Client administration, `/authorize`, `/oauth/device_authorization`, and their `/device`, `/device/decision`, and `/consent` continuations then return content-negotiated `503 feature_unavailable` before body reading, identity or client lookup, rate limiting, cleanup, or mutation. Administrator, authorization, and device controls disappear from service and session representations. This does not disable the private reserved client used for AittaDB's own signed-in session and storage operations; token-lifecycle and discovery gates are documented with the tasks that own them.
