...
Leave `FEATURE_OAUTH_APPS_ENABLED=false` until downstream third-party OAuth applications are intentionally enabled. Client administration, `/authorize`, `/oauth/device_authorization`, their browser continuations, `/oauth/token`, `/oauth/revoke`, and `/oauth/introspect` then reject before body reading, identity, CORS/client/credential lookup, rate limiting, cleanup, or mutation. Application resources return content-negotiated `503 feature_unavailable`; OAuth POSTs return no-store `temporarily_unavailable`. Their service controls disappear, and discovery omits revocation and introspection endpoints. This does not disable the private reserved client used for AittaDB's own signed-in session and storage operations; the remaining UserInfo/discovery gate is documented with the task that owns it.
