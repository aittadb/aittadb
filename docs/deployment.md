...
Sites applies D1 migrations from the packaged deployment artifact. Migration `0004_security_indexes.sql` adds cleanup/origin indexes and scrubs legacy short device user-code display values; only the hash remains durable. Migration `0005_admin_submission_results.sql` adds hash-only, expiring one-time administrator submission/result gates and stores no result or client-secret plaintext. Migrations `0006` and `0007` add and index internal file-repair state. Migration `0009_account_credential_purge.sql` indexes finite subject purge phases and adds nullable subject ownership for new revoked-JTI rows; legacy rows remain unattributed and are deliberately retained. Migration `0010_account_deletion_finalization.sql` adds in-flight R2 write fences, subject-retirement write triggers, and fail-closed clean-completion triggers. Repair/fence state contains no logical filename, content, credential, secret, or error text and has no caller-facing route. Durable requests schedule at most 25 repair candidates per minute through `waitUntil`; referenced bytes are retained and transient failures remain queued. A healthy request path must never create or alter tables. After deployment, run D1 `PRAGMA foreign_key_check`, confirm all ten migrations applied in order, and verify `/session` plus current-session storage. A missing reserved browser-client migration causes those focused browser operations to fail closed instead of creating schema or client rows at request time.

The account-deletion coordinator is internal request-time work; this task adds no public initiation or status operation. Each invocation claims one job for at most five minutes and advances finite fence, credential, record, and file batches. Operations may require repeated invocations. Do not delete or rewrite `account_deletion_jobs` tombstones: the retained former UUID is the documented pseudonymous idempotency/security exception that permanently rejects old credentials. Existing redacted audits retain their 90-day policy and hashed rate keys their five-minute policy. Monitor retryable jobs and repair backlog without logging subjects, emails, client IDs, logical/physical keys, credentials, or repair details. A foreign-owner object reference intentionally prevents finalization and requires private data-integrity investigation; never bypass the clean-finalization predicates.
