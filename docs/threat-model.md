...
- Browser storage-token leakage: forms use POST bodies with no token query parameter, results omit submitted tokens, and no cookie, local storage, session storage, or hidden follow-up form retains bearer values.
